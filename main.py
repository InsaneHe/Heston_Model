...
def main():
    ...
    date_start_train = 20161231  # 指定待训练数据的开始日期
    date_end_train = 20191231  # 指定待训练数据的结束日期
    date_start_test = 20191231  # 指定验证数据的开始日期
    date_end_test = 20201231  # 指定验证数据的结束日期

    # 选择训练数据
    train_data = data[
        (data["交易日期"] >= date_start_train) & (data["交易日期"] <= date_end_train)
    ]
    train_data = train_data[train_data["看涨看跌类型"] == "C"]
